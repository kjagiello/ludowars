 ...
 * @author kjagiello
 */
public class LudoRepresentation extends EntityRepresentation {
    boolean created = false;
    AnimatedSpriteSheet sprite;

    public LudoRepresentation() {
        Texture temp = new Texture(Gdx.files.internal("assets/images/IceBall.png"));
    ...
    public void render(ludowars.model.State S, SpriteBatch batch, ShapeRenderer sr, com.badlogic.gdx.graphics.OrthographicCamera camera) {
        if (!created) {
            Sound s = Gdx.audio.newSound(Gdx.files.internal("assets/sounds/arrow.wav"));
            s.play();
            created = true;
        }
